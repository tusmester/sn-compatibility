...
namespace SenseNet.Portal.Wall
{
    ...
    public class PostInfo
    {
        ...
        public static PostInfo CreateFromCommentOrLike(Node commentOrLike)
        {
            ...
            postInfo.Action = SNSR.GetString(commentOrLike.NodeType.Name == "Comment"
                ? Compatibility.SR.Wall.CommentedOnAContent
                : Compatibility.SR.Wall.LikesAContent);
        ...
        public PostInfo(Node node)
        {
            ...
            if (SharedContent != null) {
                ...
                if (ws != null)
                    Action = string.Format("{2} <a href='{0}'>{1}</a>", ws.Path, Content.Create(ws).DisplayName, SNSR.GetString(Compatibility.SR.Wall.What_To));
            }
        }
