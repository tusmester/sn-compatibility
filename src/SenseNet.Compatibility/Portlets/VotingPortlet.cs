...
namespace SenseNet.Portal.Portlets
{
    ...
    {
        private const string VotingPortletClass = "VotingPortlet";
        ...
        private bool UserVoted
        {
            get
            {
                return ContentQuery.Query("+TypeIs:votingitem +CreatedById:@0 +InTree:@1 .AUTOFILTERS:OFF .COUNTONLY",
                    new QuerySettings { EnableAutofilters = FilterStatus.Disabled },
                    User.Current.Id, ContextNode.Path).Count > 0;
            }
        }
        ...
        [Personalizable(true)]
        [WebCategory(EditorCategory.UI, EditorCategory.UI_Order)]
        [LocalizedWebDisplayName(VotingPortletClass, "Prop_VotingPortletMode_DisplayName")]
        [LocalizedWebDescription(VotingPortletClass, "Prop_VotingPortletMode_Description")]
        [WebOrder(50)]
        ...
        [Personalizable(true)]
        [WebCategory(EditorCategory.UI, EditorCategory.UI_Order)]
        [LocalizedWebDisplayName(VotingPortletClass, "Prop_EnableLifeSpan_DisplayName")]
        [LocalizedWebDescription(VotingPortletClass, "Prop_EnableLifeSpan_Description")]
        [WebOrder(50)]
        ...
        [Personalizable(true)]
        [WebCategory(EditorCategory.UI, EditorCategory.UI_Order)]
        [LocalizedWebDisplayName(VotingPortletClass, "Prop_ViewsOrder_DisplayName")]
        [LocalizedWebDescription(VotingPortletClass, "Prop_ViewsOrder_Description")]
        [WebOrder(50)]
        ...
        [Personalizable(true)]
        [WebCategory(EditorCategory.UI, EditorCategory.UI_Order)]
        [LocalizedWebDisplayName(VotingPortletClass, "Prop_FolderingEnabled_DisplayName")]
        [LocalizedWebDescription(VotingPortletClass, "Prop_FolderingEnabled_Description")]
        [WebOrder(50)]
        ...
        [Personalizable(true)]
        [WebCategory(EditorCategory.UI, EditorCategory.UI_Order)]
        [LocalizedWebDisplayName(VotingPortletClass, "Prop_DecimalsInResult_DisplayName")]
        [LocalizedWebDescription(VotingPortletClass, "Prop_DecimalsInResult_Description")]
        [WebOrder(50)]
        ...
        protected override void CreateChildControls()
        {
            ...
            if (this.EnableLifeSpan)
            {
                ...
                if (DateTime.UtcNow < validFrom || DateTime.UtcNow >= validTill)
                {
                    AddInvalidView();
                    return;
                }
        ...
        private void AddVotingView()
        {
            ...
            {
                Node votingView = null;

                // elevation: we do not want to grant Open permission to the 
                // user for the view, so we need to load it in elevated mode
                using (new SystemAccount())
                {
                    votingView = votingNode.VotingPageContentView;
                }

                if (votingView != null)
                {
                    var contentView = ContentView.Create(_currentContent, Page, ViewMode.Browse, votingView.Path);
                    if (contentView == null)
                    {
                    ...
                    if (!SecurityHandler.HasPermission(votingItemNewContentView.ContentHandler.ParentId, PermissionType.AddNew))
                    {
        ...
        {
            var folderName = DateTime.UtcNow.ToString("yyyy_MM_dd");
        ...
        private void AddResultView()
        {
            ...
            if (!IsResultAvalaibleBefore && VotingPortletMode == PortletMode.ResultOnly)
            {
                ...
                using (new SystemAccount())
                {
                    c = Content.Create(votingNode.CannotSeeResultContentView);
                }
            ...
            {
                Node resultView = null;

                // elevation: we do not want to grant Open permission to the 
                // user for the view, so we need to load it in elevated mode
                using (new SystemAccount())
                {
                    resultView = votingNode.ResultPageContentView;
                }

                if (resultView != null)
                {
                    var contentView = ContentView.Create(_currentContent, Page, ViewMode.InlineNew, resultView.Path) as VotingResultContentView;
        ...
        private void MyLinkBClick(object sender, EventArgs e)
        {
            Controls.Clear();
        ...
        private void VotingItemNewContentViewUserAction(object sender, UserActionEventArgs e)
        {
            // If the button's action is not Save
            if (e.ActionName != "Save") return;

            if (!SecurityHandler.HasPermission(e.ContentView.ContentHandler.ParentId, PermissionType.AddNew))
            {
