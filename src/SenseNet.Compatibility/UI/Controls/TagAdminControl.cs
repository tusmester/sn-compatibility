...
namespace SenseNet.Portal.Portlets.Controls
{
    ...
    public class TagAdminControl : UserControl
    {
        ...
        public string GetActionLink(int tagId, string action)
        {
            ...
            var queryString = Request.QueryString.Count > 0
                ? "&" + Request.QueryString
                : string.Empty;
            ...
            switch (action)
            {
                case "UpdateBlacklist":
                    ...
                    {
                        return "<a href=\"" + tmpNode.Path + "?Action=UpdateBlacklist&amp;Do=Remove&amp;Paths=" + HttpUtility.UrlEncode(searchPathParam) + "&amp;back=" +
                               $"{Portal.Page.Current.Path}{queryString}" + "\">Remove from blacklist</a>";
                    }

                    return "<a href=\"" + tmpNode.Path + "?Action=UpdateBlacklist&amp;Do=Add&amp;Paths=" + HttpUtility.UrlEncode(searchPathParam) + "&amp;back=" +
                           $"{Portal.Page.Current.Path}{queryString}" + "\">Add to blacklist</a>";
                case "Edit":
                    return
                        $"<a href=\"{tmpNode.Path}?Action=Edit&amp;Paths={HttpUtility.UrlEncode(searchPathParam)}&amp;back={Portal.Page.Current.Path}{queryString}\">Edit</a>";
                case "Delete":
                    return
                        $"<a href=\"{tmpNode.Path}?Action=Delete&amp;Paths={HttpUtility.UrlEncode(searchPathParam)}&amp;back={Portal.Page.Current.Path}{queryString}\">Delete</a>"; ;
                default:
        ...
        /// </remarks>
        private void BtnAddTagClick(object sender, EventArgs e)
        {
            ...
            foreach (var name in names)
            {
                try
                {
                    if (!TagManager.IsInRepository(name, TagPath))
                    {
                ...
                {
                    SnLog.WriteWarning("Error during tag creation. Tagname: " + name + ". Error: " + ex);
                }
            }
            Response.Redirect($"{Portal.Page.Current.Path}?{Request.QueryString}");
        }
        ...
        private void SetDataSource()
        {
            ...
            foreach (var item in ContentQuery.Query($"+TypeIs:Tag +InTree:{TagPath}", QuerySettings.AdminSettings).Nodes.ToList())
            {
                var black = GetIsBlackListed(item.Id);

                dt.Rows.Add(item.DisplayName, item.CreatedBy, item.CreationDate, item.ModificationDate,
                    refCounts.ContainsKey(item.DisplayName) ? refCounts[item.DisplayName] : 0, item.Path, black,
