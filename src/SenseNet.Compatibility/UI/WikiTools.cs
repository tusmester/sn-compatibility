...
namespace SenseNet.Portal.UI
{
    ...
    public static class WikiTools
    {
        ...
        public static string ConvertWikilinksToHtml(string articleText, Node parent)
        {
            ...
            while (true)
            {
                ...
                using (new SystemAccount())
                {
                    ...
                    var wikiQuerySettings = new QuerySettings {EnableAutofilters = FilterStatus.Disabled, Top = 1};
                    var articleQuery = wikiWs == null
                        ? ContentQuery.CreateQuery(Compatibility.SafeQueries.WikiArticlesByDisplayName, wikiQuerySettings, wTitle)
                        : ContentQuery.CreateQuery(Compatibility.SafeQueries.WikiArticlesByDisplayNameAndSubtree, wikiQuerySettings, wTitle, wikiWs.Path);
                ...
                using (new SystemAccount())
                {
                    if (SearchManager.ContentQueryIsAllowed)
                        targetArticle = ContentQuery.Query(Compatibility.SafeQueries.WikiArticlesByPath,
                            new QuerySettings { EnableAutofilters = FilterStatus.Disabled, Top = 1 },
        ...
        {
            return $"<a href=\"{path}\" class=\"sn-wiki-link\">{linkText}</a>";
        }

        private static string GetAddLinkHtml(Node parent, string articleTitle, string linkText)
        {
            return
                $"<a href=\"{Helpers.Actions.ActionUrl(Content.Create(parent), "Add", false, new {ContentTypeName = "WikiArticle", DisplayName = HttpUtility.UrlEncode(articleTitle), backtarget = "newcontent"})}\" class=\"sn-wiki-add\">{linkText}</a>";
        }
        ...
        private static string GetWikiLink(string wikiTitle, string linkText)
        {
            ...
            return wikiTitle != linkText && !string.IsNullOrEmpty(linkText) ? $"[[{wikiTitle}|{linkText}]]"
                : $"[[{wikiTitle}]]";
        }    
